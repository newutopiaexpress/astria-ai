...
export const HowToSteps: React.FC<HowToStepsProps> = ({ steps }) => {
  ...
  return (
    <div className="container backdrop-blur-md mx-auto px-4 relative text-center py-6 md:py-16">
      {/*<Badge variant="outline" className="mx-auto">How it Works?</Badge>*/}
      <h1 className="text-center mb-24 text-stone-800 font-thin tracking-tight drop-shadow-sm [text-wrap:balance] text-5xl leading-[2.7rem] md:text-6xl md:leading-[3.8rem]">
      ...
      <div className="mx-auto flex flex-col gap-9 md:flex-row justify-between items-start md:items-center relative">
        {steps.map((step) => (
        ...
        <div className="absolute top-6 left-2 right-0 hidden md:block">
          <div className="h-px bg-gradient-to-r from-white/0 from-10% via-rose-900/10 via-30% to-white/0 to-90%" aria-hidden="true"></div>
        </div>
        {/*{Spline && <Spline className="absolute top-0" scene="https://prod.spline.design/D2jez6cdpXZmTCmu/scene.splinecode" />}*/}
      </div>
    </div>
  );
};